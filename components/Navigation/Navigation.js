...
const Navlink = ({link, activeLink, onClick}) => {
  return (
    <div
      className={`${link.label === activeLink ? 'opacity-100' : 'opacity-70'} cursor-pointer text-soft-black capitalize text-sm font-normal font-sans hover:opacity-100`} 
...
const Hamburger = ({visibleModal, setVisibleModal}) => {
  ...
  return (
    <div onClick={handleClick} className="w-9 h-9 rounded-full border-solid border-2 border-soft-black opacity-30 cursor-pointer flex flex-col justify-evenly p-2">
      <div className="h-[2px] bg-soft-black"></div>
      <div className="h-[2px] bg-soft-black"></div>
      <div className="h-[2px] bg-soft-black"></div>
    </div>
  )
